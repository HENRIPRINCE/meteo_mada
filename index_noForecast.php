...
<html>
	...
	<div id="id-position_scroll">
		<img  id="id-img_timerClose" src="outils/fermer.png" />
		<div id="id-scroll_T">
			<img  id="id-img_timer" src="outils/timer_clock.png" />
			<span class="cls-span_scroll">Minuterie pour les données </span>
			<div id="id-divScroll_cover"></div>
			<span id="span-pansement"></span>
			<div id="id-divScroll" onscroll="func_scroll_value()">
				<div id="content"> </div>
			</div>
		</div>
		<div id="id-scroll_M">
		<input type="hidden" id="id-val_timerSecond" value ="1" />
			<p id="demo">Temps estimé 1 s</p>
		</div>
		<div id="id-scroll_B">
			<button class="btn btn-primary btn-block" id="id_boutChargerTimer">Charger</button>
		</div>
	</div>
	...
	<img class="cls-img_meteo" id="id-img_meteo_Timer" src="outils/meteo_timer.png" title="Afficher la minuterie pour les données"  />	
	...
	<script>
		...
		var timer_value = 1; // 1s
		var timer_Get_IconRegion = 1000; // 10 000 ms
		var timer_trater_owm  = 2000; // 20 000 ms
		var timer_showPage_GE  = 5000; // 12 000 ms
		var temps_total = 1000 * 22;
		var map_ge;
		var markers_ge_client = [];
		var infowindow_ge_client = [];
		var myVar_Original;  
		var myVar_Icon;
		...
		function initialize()
		{
			...
			var myInfosAdmin = document.createElement('DIV');
			 ...
			 //--check if map loaded
			 if (!google.maps.event.addListenerOnce(map_ge, 'tilesloaded', load_FirstOwmData)){
...
//--------------------------------------------------------document ready -----------------------------------------------------//	
		$(document).ready(function(){
			...
			//--pour charger minuature
			$("#id-img_meteo_Timer").click(function(){
				$("#id-position_scroll").toggle(1000);
			});
			
			//------reloader les données
			$("#id_boutChargerTimer").click(function(){
				$("#id-position_scroll").hide("slide", {direction: "up" }, "slow");
				
				//changer valeurs timer
				timer_value = parseInt(document.getElementById('id-val_timerSecond').value);
				timer_Get_IconRegion = timer_value * 1000; // 10 000
				timer_trater_owm  = (timer_value * 1000) * 2; // 20 000
				timer_showPage_GE  = (timer_value * 1000) + 4000; // 12 000
				temps_total = timer_value * 22 ;
				
				//---reloader les données
				 MapsApiLoaded();
			});
			//------close timer
			$("#id-img_timerClose").click(function(){
				$("#id-position_scroll").hide("slide", {direction: "up" }, "slow");
			});
		});
...
//--------------------------------------------------------CALL FUNCTION -----------------------------------------------------//
		...
		function func_scroll_value() 
		{
			var elmnt = document.getElementById("id-divScroll");
			var Vle_maxim = elmnt.scrollHeight - elmnt.clientHeight;
			var Hle_maxim = elmnt.scrollWidth  - elmnt.clientWidth;
			var x_val = elmnt.scrollLeft;
			var y_val = elmnt.scrollTop;
			var temps = parseInt((x_val * 12)/Hle_maxim);
			if (temps == 0){temps = 1};
			//document.getElementById ("demo").innerHTML =  "<br>Horizontally: " + x + "px<br>Vertically: " + y + "px" + "<br> temp : " + temps;
			document.getElementById ("demo").innerHTML ="Temps estimé : " + temps + " s";
			document.getElementById ("id-val_timerSecond").value = temps;
		}
		
		function load_FirstOwmData(){
			//load OwmData
			$("#id-position_scroll").show("slide", {direction: "up" }, "slow");
			$("#id-img_meteo_Timer").show("slide", {direction: "up" }, "slow");
		}
		
		function MapsApiLoaded(){
			//----diovina all objects
			document.getElementById("id_isa").value = "0";
			document.getElementById('id-spin_text').innerHTML = "";
			//--remove all markers
			Delete_MarkersInfowindow_client("Marker_Climat"); 
			//---afeno ny rehetra
			$("#id-img_meteo_infos").hide("slide", {direction: "up" }, "slow");
			$("#id-img_meteo_jour").hide("slide", {direction: "up" }, "slow");
			$("#id-img_meteo_previsions").hide("slide", {direction: "up" }, "slow");
			$("#id-img_meteo_HotCold").hide("slide", {direction: "up" }, "slow");
			$("#id-zone_HotCold").hide("slide", {direction: "up" }, "slow");
			$("#id-band_infos").hide("slide", {direction: "up" }, "slow");
			//------load get icon
		...
		function func_getIcons()
		{
			...
			//appel function
			myVar_Icon = setInterval(Get_IconRegion, timer_Get_IconRegion);
		}
		...
		function Get_IconRegion()
		{
			...
			var region_reste = 22 -  isa_eto;
			document.getElementById('id-spin_text').innerHTML = "Chargement de données (par " +  timer_value + " s. durée totale : " + temps_total + " s) <br> Région : " + nom_region +  " <br> (Reste à télécharger :  " + region_reste + " région(s))";
...
//-----------------------------------------------------------------------------------------FIN sending ajax-----------------------------------//
			...
			if (val_final >= 22)
			{
				...
				setTimeout(trater_owm, timer_trater_owm);
			}
		...
		function trater_owm()
		{
			...
			//--call function
			myVar_GE = setTimeout(showPage_GE, timer_showPage_GE);
		...
		function showPage_GE()
		{
			...
			for (var i = 0; i < iNbre_pts; i++)
			{
				...
				//stocker marker
				markers_ge_client.push(Marker_google);
		...
		function ampitaoHafatra(marker, ValcontentString) 
		{
			...
			//--stocker  infos
			infowindow_ge_client.push(infowindow);
		}
		...
		function Delete_MarkersInfowindow_client(layerName_client) 
		{
			for (var i = 0; i < markers_ge_client.length; i++) {
					if ( markers_ge_client[i].name == layerName_client){
						markers_ge_client[i].setMap(null);
						//markers_ge_client[i].setVisible(false);
						//close infos client
						infowindow_ge_client[i].close();
						
						//alana tanteraka
						var index_marker = markers_ge_client.indexOf(i);
						var index_infowindow = infowindow_ge_client.indexOf(i);
						if (index_marker > -1){
							markers_ge_client.splice(index_marker, 1)
						}
						if (index_infowindow > -1){
							infowindow_ge_client.splice(index_infowindow, 1)
						}
					}
				}
